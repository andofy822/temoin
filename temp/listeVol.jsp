                    ...
                    <% if (vols != null && !vols.isEmpty()) {
                        for (VolDAOCpl vol : vols) { %>
                            <tr>
                                ...
                                <td><a href="<%=request.getContextPath()%>/home/promotionForm?idVol=<%=vol.getId()%>&idTypeSiege=<%=vol.getIdtypesiege()%>">Promotion</a></td>
                                <td><a href="<%=request.getContextPath()%>/home/annulationTempsForm?idVol=<%=vol.getId()%>">Heure annulation</a></td>
                                <td><a href="<%=request.getContextPath()%>/home/reservationTempsForm?idVol=<%=vol.getId()%>">Heure reservation</a></td>
                            </tr>
                        <% }
                    } else { %>
                        <tr>
                            <td colspan="15">Aucun vol disponible</td>
                        </tr>
                    <% } %>
                </tbody>
            </table>
